...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nombreNegocio) || empty($duenoId)) {
        ...
        if (empty($negocioId)) {
            // Add new Negocio
            $sql = "INSERT INTO Negocio (DueñoId, NombreNegocio) VALUES (?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("is", $duenoId, $nombreNegocio);
        } else {
            // Update existing Negocio
            $sql = "UPDATE Negocio SET DueñoId = ?, NombreNegocio = ? WHERE NegocioId = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isi", $duenoId, $nombreNegocio, $negocioId);
        }
        ...
        <div class="col">
            ...
            if (isset($_GET['database'])) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<thead><tr><th>Negocio ID</th><th>Dueño ID</th><th>Nombre Negocio</th><th>Actions</th></tr></thead>";
                    echo "<tbody>";
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr><td>" . $row["NegocioId"] . "</td><td>" . $row["DueñoId"] . "</td><td>" . $row["NombreNegocio"] . "</td>";
